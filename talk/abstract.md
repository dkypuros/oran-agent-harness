...
A remediation routing rule that follows O-RAN's resource layering, sending service-layer actions (cell re-home, RAN parameter change) upward to partner orchestrators via intent interfaces, and infrastructure actions (firmware, kernel config, PTP tuning) downward through O2 IMS.

A guardrail contract layer specifying operator-harness co-authorization, blast-radius caps, dry-run defaults, and structured audit emission.
