...
import { type NetworkFunctionStatus } from '@/lib/api'
...
function NFCard({ data }: { data: NFCardData }) {
...
export function NetworkTopologyView({
  ...
  useEffect(() => {
    if (!networkFunctions.length) return
    const positions: Record<string, { x: number; y: number }> = {
      ...
      'gnb-cu': { x: 50, y: 200 },
      'gnb-du': { x: 50, y: 50 },
      'bf3-firewall': { x: 800, y: 400 },
      'data-network': { x: 1000, y: 400 },
    }

    const syncNodes = (networkFunctions: NetworkFunctionStatus[]) => {
      setNodes((existing) =>
        existing.map((node) => {
          const status = networkFunctions.find((nf) => nf.id === node.id)
          ...
          return {
            ...
            data: {
              ...
              load: status.load,
              sessions: showActiveSessions ? status.sessions : undefined,
              throughput: status.throughput,
              selected: status.label === selectedNF,
            },
          }
        })
      )
    ...
    syncNodes(networkFunctions)
  }, [bridgeConnected, networkFunctions, selectedNF, setNodes, showActiveSessions])

  // Simulate traffic flow animation
  useEffect(() => {
    if (simulationStatus === 'running' && !bridgeConnected) {
  ...
  }, [bridgeConnected, simulationStatus, setNodes])
    ...
    <Card className="h-full">
      ...
      <div className="p-4 border-t bg-muted/30">
        <div className="flex items-center justify-between text-xs">
          ...
          <Badge variant={bridgeConnected ? 'default' : 'secondary'} className="text-xs">
            {bridgeConnected ? 'Bridge Live' : 'Fallback Mock Data'}
          </Badge>
