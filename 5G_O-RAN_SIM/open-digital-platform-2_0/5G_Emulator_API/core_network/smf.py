...
upf_pfcp_addr = None  # UPF's PFCP endpoint (ip, 8805) for real mode

# Protocol mode: "rest" (default) or "real" (PFCP over UDP 8805)
...
async def _handle_pfcp_response(msg_type: int, seid: int, seq: int,
                                 ie_data: bytes, addr):
    """Handle incoming PFCP response messages from UPF."""
    logger.info(f"SMF PFCP: Received response msg_type={msg_type}, "
                f"SEID={seid}, seq={seq}, from={addr}")


async def _send_pfcp_real(msg_type: int, seid: int, upf_addr=None):
    """Send PFCP message to UPF via real UDP transport."""
    global _pfcp_seq_counter
    if _pfcp_transport and upf_pfcp_addr:
        dest = upf_addr or upf_pfcp_addr
        seq = await _pfcp_transport.send_message(msg_type, seid, b'', dest)
        logger.info(f"SMF PFCP: Sent msg_type={msg_type}, SEID={seid}, "
                    f"seq={seq}, to={dest}")
        return seq
    return 0
...
async def lifespan(app: FastAPI):
    global upf_url, upf_pfcp_addr, _pfcp_transport
    # Startup
    nf_registration = {
        "nf_type": "SMF",
        "ip": "127.0.0.1",
    ...
    try:
        ...
        if 'message' in upf_info:
            ...
            upf_ip = upf_info.get('ip', '127.0.0.1')
            upf_pfcp_addr = (upf_ip, 8805)  # PFCP standard port
    ...
    if PROTOCOL_MODE == "real":
        try:
            ...
            from core_network.transport import (PfcpTransport,
                                                 PFCP_ASSOCIATION_SETUP_REQUEST)
            # Bind to a different port than UPF (UPF uses 8805, SMF uses ephemeral)
            _pfcp_transport = PfcpTransport(bind_port=0)  # OS-assigned port
            await _pfcp_transport.start(_handle_pfcp_response)

            # Send PFCP Association Setup to UPF
            if upf_pfcp_addr:
                await _pfcp_transport.send_message(
                    PFCP_ASSOCIATION_SETUP_REQUEST, 0, b'', upf_pfcp_addr)
                logger.info(f"SMF: PFCP Association Setup sent to UPF at {upf_pfcp_addr}")

            logger.info("SMF: Real PFCP transport started")
        except Exception as e:
            logger.error(f"SMF: Failed to start PFCP transport: {e}")
            logger.info("SMF: Falling back to REST-only mode")

    yield

    # Shutdown PFCP transport
    if _pfcp_transport:
...
def _send_pfcp_establishment_request(pdu_session: dict) -> dict:
    ...
    In real mode, also sends a binary PFCP message via UDP port 8805.
    """
    global upf_url
    if not upf_url:
    ...
    # In real mode, also send via actual PFCP UDP transport
    if PROTOCOL_MODE == "real" and _pfcp_transport:
        try:
            from core_network.transport import PFCP_SESSION_ESTABLISHMENT_REQUEST
            seid = abs(hash(str(pdu_session.get('pduSessionId', 0)))) & 0xFFFFFFFFFFFFFFFF
            asyncio.get_event_loop().create_task(
                _send_pfcp_real(PFCP_SESSION_ESTABLISHMENT_REQUEST, seid))
            logger.info(f"SMF: PFCP Session Establishment also sent via real UDP transport")
        except Exception as e:
            logger.warning(f"SMF: Real PFCP send failed (REST fallback active): {e}")
...
async def transport_stats():
    ...
    if _pfcp_transport:
        ...
        result["upf_pfcp_addr"] = str(upf_pfcp_addr)
    return result

@app.get("/health")
def health_check():
    """Health check endpoint for SMF - TS 29.502"""
    return {
        "status": "healthy",
        "service": "SMF",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="SMF - Session Management Function")
    parser.add_argument("--host", default="0.0.0.0", help="Host to bind to")
    parser.add_argument("--port", type=int, default=get_port("smf"), help="Port to bind to")
    parser.add_argument("--protocol-mode", choices=["rest", "real"], default="rest",
                        help="Transport mode: rest (HTTP only) or real (PFCP over UDP)")
