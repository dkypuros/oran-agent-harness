...
\node[bridge] at (4.0, 1.45)  {B3\\ 2+3+8+9};
...
\end{tikzpicture}
\caption{Seven contract bridges between five standards bodies and the harness enforcement plane. Each numbered bridge corresponds to a subsection of this section. Specific interface references appear under each standards body box. The enforcement plane below the rail is where each bridge becomes machine-verifiable via the citation header on the relevant authored YAML or JSON artifact.}
\label{fig:bridges}
\end{figure*}

Each bridge below names the from-spec, the to-spec, the seam artifact in the harness, the file anchor where the bridge is implemented, and the bibliography references involved. Figure~\ref{fig:bridges} shows the seven bridges in one view. A reviewer can confirm any single bridge by reading the named files.
...
This is the architectural core of the harness. The routing rule at \texttt{harness/routing-rules/contribution-1-routing-rule.yaml} dispatches infrastructure-layer faults through the O2 IMS provisioning surface \cite{oran_o2_ganp,oran_o2_ims} into Kubernetes-native CRD reconcilers. The reference O2 IMS implementation is Red Hat's O-Cloud Manager \cite{oran_o2ims_ocm}. The bridge produces what the source repository calls the 2+3+8+9 conjunction (numbers from the AMA-numbered bibliography in \texttt{docs/references.md}, not from this paper's rendered bibliography): a single \texttt{RemediationProposal} travels through the O-RAN-standardized provisioning request envelope into either Metal3 HostFirmwareComponents \cite{metal3_bmo} or the Machine Config Operator \cite{openshift_mco}. Kernel Module Management v1beta1 \cite{kmm_operator} is a co-delivery path within the same bridge for driver-swap action classes; the four-element conjunction names the two CRDs that anchor the novelty claim against publicly-published architectures, with KMM as a third concrete reconciler that the same routing surface dispatches into.
