...
\node[l3] (l3) at (0, 2.2) {\textbf{Layer 3: Bidirectional conformance index}\\ \scriptsize verify gate check 4 (conformance\_complete)};
...
\end{tikzpicture}
\caption{Four-layer claim anchoring system (read bottom-up). Each layer is mechanically enforced by a specific check of the verify gate. Layer 1 turns documentation into a contract; Layer 4 turns architectural claims into mechanically falsifiable assertions against committed expected outputs.}
\label{fig:fourlayer}
\end{figure}
...
\end{tikzpicture}
\caption{Ten-check verify gate as a three-phase pipeline. Phase A enforces hygiene (em-dash audit, leakage guard, README structure, file counts). Phase B enforces structural integrity (citation headers, JSON and YAML parse, conformance index). Phase C enforces spec conformance (JSON Schema validation, end-to-end walker replay). Any single check failure exits the gate non-zero with a specific failure row.}
\label{fig:verifygate}
\end{figure*}

The gate is a single Python script at \texttt{scripts/verify.py}. It runs ten checks in sequence and exits 0 on 10 of 10 pass, non-zero with specific failure rows otherwise. Figure~\ref{fig:verifygate} shows the three-phase pipeline. Each check guards a specific failure mode.
